...
void compare(int ch, char* run1, char* run2);

int main(int argc, char *argv[]){
    
  for(int ch = 0; ch < 26; ch++) compare(ch, argv[1], argv[2]);
  return 1;
}

void compare(int ch, char* run1, char* run2){
  TString filename1;
  filename1.Form("./%s/rms_%d.root", run1, ch);
  
  TString filename2;
  filename2.Form("./%s/rms_%d.root", run2, ch);
  ...
  if(atoi(run1) < atoi(run2)) canvasName.Form("./%svs%s/cmp_%d.png", run1, run2, ch);
  else canvasName.Form("./%svs%s/cmp_%d.png", run2, run1, ch);
  c->SaveAs(canvasName.Data());
  
  if(atoi(run1) < atoi(run2)) canvasName.Form("./%svs%s/cmp_%d.root", run1, run2, ch);
  else canvasName.Form("./%svs%s/cmp_%d.root", run2, run1, ch);
  c->SaveAs(canvasName.Data());
}